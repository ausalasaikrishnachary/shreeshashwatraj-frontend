...
const KachaPurchaseInvoiceForm = ({ user }) => {
  ...
  const handlePreview = () => {
    if (!isPreviewReady) {
      window.alert(
        "Please submit the purchase invoice first to generate preview",
      );
      return;
    }
  ...
  const calculateTotals = () => {
    ...
    discountCharges.forEach((charge) => {
      ...
      if (charge.calculationType === "percentage") {
        discountChargesTotal +=
          (taxableAmount + additionalChargeAmount) * (amount / 100);
      } else {
    ...
    const actualTotal =
      taxableAmount + additionalChargeAmount - discountChargesTotal;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(
        "Submitting kacha purchase invoice payload with invoice number:",
                        ...
                        onChange={(e) => {
                          ...
                          if (!e.target.value) {
                            setItemForm((prev) => ({
                              ...
                              discount:
                                parseFloat(
                                  invoiceData.supplierInfo?.discount,
                                ) || 0,
                              batch: "",
                              batch_id: "",
                            }));
                    ...
                    {batches.length > 0 && itemForm.maintain_batch !== 0 && (
                      <Form.Select
                        ...
                        onChange={(e) => {
                          ...
                          const supplierDiscount =
                            parseFloat(invoiceData.supplierInfo?.discount) || 0;
                          const selectedProduct = products.find(
                            (p) => p.id === itemForm.product_id,
                          );
