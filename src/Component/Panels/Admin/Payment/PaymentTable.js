...
//   };
  ...
  const receiptStats = [
    { label: 'Total Payments', value: '₹ 2,50,000', change: '+18%', type: 'total' },
  ...
  const fetchNextReceiptNumber = async () => {
    try {
      console.log('Fetching next receipt number from:', `${baseurl}/api/next-Payment-number`);
      const response = await fetch(`${baseurl}/api/next-Payment-number`);
      if (response.ok) {
        const data = await response.json();
  ...
  const generateFallbackReceiptNumber = async () => {
    try {
      ...
      const response = await fetch(`${baseurl}/api/last-payment`);
      if (response.ok) {
        const data = await response.json();
        if (data.lastReceiptNumber) {
          const lastNumber = data.lastReceiptNumber;
          const numberMatch = lastNumber.match(/PAY(\d+)/);
          if (numberMatch) {
            const nextNum = parseInt(numberMatch[1], 10) + 1;
            const fallbackReceiptNumber = `PAY${nextNum.toString().padStart(3, '0')}`;
  ...
  const fetchReceipts = async () => {
    try {
      setIsLoading(true);
      console.log("Fetching receipts from:", `${baseurl}/api/receipts?data_type=Payment`);

      const response = await fetch(`${baseurl}/api/receipts?data_type=Payment`);
        ...
        <div className="receipts-content-area">
          ...
          <div className="receipts-actions-section">
            <div className="quotation-container p-3">
              <h5 className="mb-3 fw-bold">View Payments</h5>
              {/* Filters and Actions */}
