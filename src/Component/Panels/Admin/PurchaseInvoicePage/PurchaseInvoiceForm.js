...
const CreateProductInvoice = ({ user }) => {
  ...
  const handlePreview = () => {
    if (!isPreviewReady) {
      window.alert(
        "⚠️ Please submit the purchase invoice first to generate preview",
      );
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(
        "Submitting purchase invoice with number:",
        finalInvoiceNumber,
      );
      ...
      const missingProductIds = invoiceData.items.filter(
        (item) => !item.product_id,
      );
      const missingBatchIds = invoiceData.items.filter(
        (item) => !item.batch_id,
      );
      ...
      const payload = {
        ...
        discount_charges_amount: discountCharges.reduce(
      ...
      if (!payload.product_id || !payload.batch_id) {
        console.error(
          "❌ CRITICAL: product_id or batch_id is still undefined in payload!",
        );
        console.error("Payload structure:", JSON.stringify(payload, null, 2));
      } else {
        console.log(
          "✅ SUCCESS: product_id and batch_id are properly set in payload!",
        );
      }

      console.log(
        "Submitting purchase invoice payload with invoice number:",
      ...
      if (!response.ok) {
        throw new Error(
          responseData.error || "Failed to submit purchase invoice",
        );
      }
    ...
    if (itemToEdit.product_id) {
      try {
        const res = await fetch(
          `${baseurl}/products/${itemToEdit.product_id}/batches`,
        );
        if (res.ok) {
          const batchData = await res.json();
          setBatches(batchData);
            ...
            {invoiceData.supplierInfo.state && (
              ...
              >
                ...
                {isSameState() ? (
                  ...
                  <>
                    IGST (Inter-State: {invoiceData.companyInfo.state} to{" "}
                    {invoiceData.supplierInfo.state})
                  </>
                )}
              </Alert>
              ...
              <Row className="mb-3 company-info bg-white p-3 rounded">
                <Col md={8}>
                  <div>
                    ...
                    <br />
                    State Name : {invoiceData.companyInfo.state}, Code :{" "}
                    {invoiceData.companyInfo.stateCode}
                  </div>
                </Col>
                <Col md={4}>
                    ...
                    {!selected ? (
                                    ...
                                    <div
                                      ...
                                      onClick={() => {
                                        ...
                                        setInvoiceData((prev) => ({
                                          ...
                                          billingAddress: {
                                            addressLine1:
                                              acc.billing_address_line1,
                                            addressLine2:
                                              acc.billing_address_line2 || "",
                                          ...
                                          shippingAddress: {
                                            addressLine1:
                                              acc.shipping_address_line1,
                                            addressLine2:
                                              acc.shipping_address_line2 || "",
                      ...
                      <>
                        ...
                        <div className="bg-light p-2 rounded">
                          ...
                          <div>
                            <strong>GSTIN:</strong>{" "}
                            {invoiceData.supplierInfo.gstin}
                          </div>
                          <div>
                            <strong>State:</strong>{" "}
                            {invoiceData.supplierInfo.state}
                        ...
                        >
                          ...
                          <div>
                            {products
                              .filter((p) => {
                                const groupMatch =
                                  p.group_by === "Purchaseditems" ||
                                  p.can_be_sold === true;
                                const typeMatch = p.product_type === "PAKKA";
                                const searchMatch = p.goods_name
                                  .toLowerCase()
                              ...
                              .map((p) => {
                                ...
                                const supplierDiscount =
                                  parseFloat(supplierAccount?.discount) || 0;
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-1">
                      ...
                      {invoiceData.retailerDiscount > 0 && (
                        <small className="text-primary">
                          Retailer: {invoiceData.retailerDiscount}%
                        </small>
                      )}
                      {invoiceData.supplierDiscount > 0 && (
                        <small className="text-primary">
                          Supplier: {invoiceData.supplierDiscount}%
                        </small>
                      )}
                    </div>
                    <Form.Control
                ...
                <Col md={5}>
                  ...
                  <Row>
                    ...
                    >
                      ...
                      <div className="mb-2 fw-bold text-danger">
                        Discount Charges:
                      </div>
                        ...
                        {discountCharges
                          .reduce((sum, c) => {
                            if (c.calculationType === "percentage") {
                              const base =
                                parseFloat(invoiceData.taxableAmount) +
                                parseFloat(invoiceData.totalGST) +
                                parseFloat(invoiceData.totalCess) +
                                parseFloat(invoiceData.additionalChargeAmount);
