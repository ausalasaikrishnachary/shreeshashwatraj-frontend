...
const handleCreateVoucher = async (row, retailerId, transactionType) => {
  ...
  try {
    const payload = {
      ...
      amount: amount,
      transaction_type: apiTransactionType, // Set proper transaction type for manual submission
      vchr_no: null, // Will be generated by backend
      ...
      status: 'approved' // Set status as approved for manual submissions
    };
...
},
{
  ...
  render: (value, row) => {
    ...
    // Filter accounts based on search term - show ALL accounts (not just Bank Accounts)
    const filteredAccounts = accounts.filter(account => {
      if (!account) return false;
      
      const searchTermLower = state.searchTerm.toLowerCase();
      const partyName = (account.party_name || "").toLowerCase();
      const businessName = (account.business_name || "").toLowerCase();
    ...
    console.log("Dropdown open state:", state.isOpen);
    ...
    return (
      <div style={{ position: "relative", width: "100%" }}>
        ...
        {/* Dropdown icon */}
        {!selectedAccount && !state.searchTerm && (
          <div
            style={{
              position: "absolute",
              right: "10px",
        ...
        {state.isOpen && (
          <>
            ...
            >
              <div style={{ padding: "8px 12px", borderBottom: "1px solid #dee2e6", fontWeight: 600, fontSize: "12px", backgroundColor: "#f8f9fa" }}>
                Select Account ({filteredAccounts.length} found)
              </div>
                ...
                filteredAccounts.map((account) => (
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: selectedAccountId === account.id.toString() ? "#f0f8ff" : "transparent",
                  ...
                  >
                    ...
                    <div style={{ fontSize: "11px", color: "#6c757d", marginTop: "3px" }}>
                      {account.role || account.group || "No Role"} 
                      {account.group === "Bank Accounts" && " 🏦"}
                    </div>
                    {account.business_name && account.business_name !== (account.party_name || account.name) && (
              ...
              <div style={{ padding: "8px 12px", borderTop: "1px solid #dee2e6", backgroundColor: "#f8f9fa" }}>
                <button
...
const handleFileUpload = async (e) => {
  ...
  reader.onload = async (e) => {
    try {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        try {
          // Prepare payload for import with status = 'pending'
          const importPayload = {
          ...
          if (selectedBankAccount) {
            importPayload.party_id = globalSelectedRetailer;
            importPayload.party_name = selectedBankAccount.business_name;
          }

          // 1. First, import the bank statement with pending status
          const importResponse = await axios.post(`${baseurl}/api/direct-deposit/import`, importPayload);

          if (importResponse.data.success) {
            importSuccessCount++;
            
            // Get the transaction_id from import response
            const transactionId = importResponse.data.transaction_id || importResponse.data.data?.transaction_id;
            
            // 2. If import successful, create voucher for this transaction
  ...
  // Remove 'vchr_no' field - backend generates it
      ...
      let resultMessage = `📊 Import Results:\n`;
      resultMessage += `✅ Import Success: ${importSuccessCount}\n`;
      resultMessage += `❌ Import Failed: ${importFailCount}\n`;
      resultMessage += `\n📝 Voucher Results:\n`;
      resultMessage += `✅ Vouchers Created (Pending): ${voucherSuccessCount}\n`;
      resultMessage += `❌ Vouchers Failed: ${voucherFailCount}`;
      ...
      setImportResults({
        ...
        importFailed: importFailCount,
        voucherSuccess: voucherSuccessCount,
        voucherFailed: voucherFailCount,
        errors: errors
      });
      
      // Refresh the bank statements table
      fetchBankStatements();
...
{importResults && (
  <div className={`alert ${importResults.importFailed === 0 && importResults.voucherFailed === 0 ? 'alert-success' : 'alert-warning'} mt-3`}>
    <strong>Import & Voucher Creation Results:</strong><br />
    <strong>Import:</strong> Total: {importResults.total} | Success: {importResults.importSuccess} | Failed: {importResults.importFailed}<br />
    <strong>Vouchers:</strong> Created (Pending): {importResults.voucherSuccess} | Failed: {importResults.voucherFailed}
