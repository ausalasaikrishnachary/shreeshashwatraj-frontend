...
const Ledger = () => {
  ...
  const groupedLedger = ledgerData.reduce((acc, entry) => {
    ...
    const transactionType = (entry.trantype || "").toLowerCase();
    ...
    else if (transactionType === "sales" || transactionType === "stock transfer" ||
      dataType === "sales" || dataType === "stock transfer") {
      acc[key].totalDebit += amount;
    }
    else if (transactionType === "CreditNote" || transactionType === "Receipt" || transactionType === "payment" ||
      dataType === "sales" || dataType === "stock transfer") {
      acc[key].totalCredit += amount;
    }
    // Fallback to original DC field
    else if (entry.DC === "C") {
      acc[key].totalCredit += amount;
    } else if (entry.DC === "D") {
      acc[key].totalDebit += amount;
  ...
  const updateBalanceInDB = async (partyID, balance) => {
    try {
      ...
      // CORRECTED LOGIC:
      // If totalCredit > totalDebit, it means this party receives money (Purchase/Stock Inward)
      // So they are our Creditor - Balance should be CREDIT
      if (totalCredit > totalDebit) {
        balance_type = "Cr";  // Purchase/Stock Inward parties - we owe them
        absBalance = totalCredit - totalDebit;
      }
      // If totalDebit > totalCredit, it means this party pays money (Sales/Stock Transfer)
      // So they are our Debtor - Balance should be DEBIT
      else {
        balance_type = "Dr";  // Sales/Stock Transfer parties - they owe us
