...
const KachaSalesInvoiceForm = ({ user }) => {
    ...
    const fetchStaffMembers = async () => {
      try {
        ...
        const staffs = data.filter(
          (acc) => acc.role === "staff" || acc.staffid,
        );
  ...
  const addItem = () => {
    ...
    if (editingItemIndex !== null) {
      ...
      if (
        ...
        selectedBatchDetails &&
        parseFloat(selectedBatchDetails.quantity) < quantity
      ) {
        window.alert(
          `⚠️ Item "${finalItem.product}" updated with insufficient stock! Negative stock created.`,
        );
      } else {
      ...
      if (
        ...
        window.alert(
          `⚠️ Item "${finalItem.product}" added with insufficient stock! Negative stock created.`,
        );
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const staffName =
        selectedStaff?.name || selectedStaff?.assigned_staff || "";
      const mobileNumber =
      ...
      const payload = {
        ...
        additional_charges_amount: charges.reduce(
      ...
      setTimeout(() => {
        navigate(
          `/kachainvoicepdf/${responseData.voucherId || editingVoucherId}`,
        );
      }, 2000);
    } catch (err) {
              ...
              <Row className="mb-3 company-info bg-white p-3 rounded">
                <Col md={8}>
                  <div>
                    ...
                    <br />
                   State Name : {invoiceData.companyInfo.state}, 
                   Code : {invoiceData.companyInfo.stateCode}
                  </div>
                </Col>
                <Col md={4}>
                    ...
                    {!selected ? (
                      ...
                      <>
                        ...
                        <div className="bg-light p-2 rounded">
                          ...
                          <div>
                            <strong>GSTIN:</strong>{" "}
                            {invoiceData.supplierInfo.gstin || "Not Available"}
                          </div>
                          <div>
                            <strong>State:</strong>{" "}
