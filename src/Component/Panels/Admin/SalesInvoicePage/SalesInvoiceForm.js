...
const CreateInvoice = ({ user }) => {
  ...
  const fetchInvoiceDataForEdit = async (voucherId) => {
    try {
      ...
      if (result.success && result.data) {
        ...
        if (
          apiData.additional_charges_type &&
          apiData.additional_charges_amount
        ) {
          setCharges([
            {
  ...
  const transformApiDataToFormFormat = (apiData) => {
    ...
    return {
      ...
      supplierInfo: {
        ...
        business_name:
          account?.business_name || apiData.business_name || "Business",
        account_name:
          account?.account_name || apiData.account_name || "Busines",
        gstin: apiData.gstin || "",
  ...
  const addItem = () => {
    ...
    if (quantity <= 0) {
      window.alert(
        "⚠️ Cannot add item - Quantity is zero. Please enter a quantity greater than 0.",
      );
      return;
    }
    ...
    if (editingItemIndex !== null) {
      ...
      if (
        ...
        window.alert(
          `⚠️ Item "${finalItem.product}" updated with ZERO stock! Negative stock created.`,
        );
      } else {
      ...
      if (
        ...
        window.alert(
          `⚠️ Item "${finalItem.product}" added with ZERO stock! Negative stock created.`,
        );
      } else {
  ...
  const editItem = (index) => {
    ...
    const productNetPrice = selectedProduct
      ? parseFloat(selectedProduct.net_price) || 0
      : itemToEdit.price;
  ...
  const calculateTotals = () => {
    ...
    const actualTotal =
      taxableAmount + totalGST + totalCess + additionalChargeAmount;
  ...
  const clearDraft = () => {
    ...
    const resetData = {
      ...
      companyInfo: {
        ...
        state: invoiceData.companyInfo?.state || "",
        stateCode: invoiceData.companyInfo?.stateCode || "",
      },
      supplierInfo: {
        name: "",
        businessName: "",
    ...
    try {
      ...
      if (!isEditMode) {
        ...
        if (invoiceNumberResponse.ok) {
          ...
          console.error(
            "Failed to fetch next invoice number, using existing:",
            finalInvoiceNumber,
          );
        }
      }
      ...
      const uniqueDocTypes = [
        ...new Set(invoiceData.items.map((item) => DocumentType(item.gst))),
      ];
      ...
      const payload = {
        ...
        business_name:
          ...
          accounts.find((acc) => acc.id === selectedSupplierId)
            ?.business_name ||
          invoiceData.supplierInfo.name,
      ...
      setTimeout(() => {
        navigate(
          `/sales/invoice-preview/${responseData.voucherId || editingVoucherId}`,
        );
      }, 2000);
    } catch (err) {
      ...
      >
        ...
        <div className="admin-content-wrapper">
          <Container fluid className="invoice-container">
            ...
            {invoiceData.billingAddress?.state && (
              <Alert
                variant={isSameState() ? "success" : "warning"}
                className="mb-3"
              >
                <strong>Tax Type: </strong>
                {isSameState() ? (
                  <>
                    CGST & SGST (Same State - Company:{" "}
                    {invoiceData.companyInfo.state}, Customer:{" "}
                    {invoiceData.billingAddress?.state})
                  </>
                ) : (
                  <>
                    IGST (Inter-State: Company: {invoiceData.companyInfo.state}{" "}
                    → Customer: {invoiceData.billingAddress?.state})
                  </>
                )}
              </Alert>
            )}
              ...
              <Row className="mb-3 company-info bg-white p-3 rounded">
                <Col md={8}>
                  <div>
                    ...
                    <br />
                    Email: {invoiceData.companyInfo.email} | Phone:{" "}
                    {invoiceData.companyInfo.phone}
                    <br />
                    GSTIN/UIN: {invoiceData.companyInfo.gstin}
                    <br />
                    State Name : {invoiceData.companyInfo.state || "Bihar"},
                    Code : {invoiceData.companyInfo.stateCode || "10"}
                  </div>
                </Col>
                <Col md={4}>
                                    ...
                                    <div
                                      ...
                                      onClick={() => {
                                        ...
                                        if (supplier) {
                                          ...
                                          const staffId =
                                            supplier.staffid || null;
                                          setSelectedStaffId(staffId);

                                          const retailerDiscount =
                                            parseFloat(supplier.discount) || 0;
                      ...
                      <>
                          ...
                          <div className="btn-group">
                            <Button
                              ...
                              onClick={() => {
                                if (selectedSupplierId) {
                                  navigate(
                                    `/retailers/edit/${selectedSupplierId}`,
                                  );
                                }
                              }}
                            >
                        ...
                        <div className="bg-light p-2 rounded">
                          ...
                          {customerType === "b2b" && (
                            <div>
                              <strong>GSTIN:</strong>{" "}
                              {invoiceData.supplierInfo.gstin || "N/A"}
                            </div>
                          )}
                          <div>
                            <strong>State:</strong>{" "}
                          ...
                          {invoiceData.supplierInfo.staffid &&
                            invoiceData.supplierInfo.staffid !== null && (
                              <div>
                ...
                <Row className="align-items-end">
                  ...
                  <Col md={2}>
                    ...
                    <div className="position-relative">
                      <input
                        ...
                        style={{
                          cursor: itemForm.product ? "pointer" : "text",
                        }}
                        title={
                          itemForm.product_id
                            ? (() => {
                  ...
                  <Col md={1} className="sales-invoice-add px-0">
                    <div className="d-flex flex-column h-100 justify-content-end">
                      <Button
                        variant={
                          editingItemIndex !== null ? "warning" : "success"
                        }
                        onClick={addItem}
                ...
                <Col md={5}>
                      ...
                      >
                        ...
                        <div style={{ width: "35%" }}>
                          ...
                          >
                            ...
                            <option value="Insurance Charge">
                              Insurance Charge
                            </option>
                            <option value="Loading Charge">
                              Loading Charge
                            </option>
                            <option value="Packing Charge">
                              Packing Charge
                            </option>
                            <option value="Other Taxes">Other Taxes</option>
                            <option value="Other Charges">Other Charges</option>
                            <option value="Reimbursements">
                              Reimbursements
                            </option>
                  ...
                  <Row>
                    ...
                    >
                      ...
                      <div className="mb-2 fw-bold text-success fs-5">
                        Grand Total:
                      </div>
                    </Col>
                    ...
                    <Col md={5} className="d-flex flex-column align-items-end">
                      ...
                      <div className="mb-2">
                        ₹
                        {parseFloat(
                          invoiceData.additionalChargeAmount || 0,
                        ).toFixed(2)}
                      </div>
